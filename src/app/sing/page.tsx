...
export default function SingPage() {
  ...
  const beginSinging = useCallback(async () => {
    ...
    try {
      ...
      timerRef.current = setInterval(() => {
        setElapsed(Math.round(ctx.currentTime - startTimeRef.current));
      }, 500);
      ...
      const detect = () => {
        ...
        const frame = Math.floor(elapsed * ctx.sampleRate / HOP);
        ...
        if (voiceHz > 0 && refFrame && refFrame.f0 > 0) {
          ...
          setCurrentNote(hzToNoteName(voiceHz));
          setCurrentDeviation(Infinity); // 无参考
        } else {
          setCurrentNote("--");
        }

        // 伴奏结束自动停止
        if (elapsed >= (backingBufferRef.current?.duration ?? 0)) {
          stopSinging();
          return;
        }
  ...
  const stopSinging = useCallback(() => {
    ...
    const result = summarize(voiceF0sRef.current, refFramesRef.current);
    ...
    setTimeout(() => router.push("/sing/results"), 800);
  }, [songTitle, elapsed, router]);
      ...
      <div className="w-full max-w-lg relative z-10">
        ...
        {state === "ready" && (
          <div className="space-y-6">
            ...
            <button onClick={() => { setState("setup"); fileRef.current && (fileRef.current.value = ""); }}
        ...
        {state === "singing" && (
          <div className="space-y-5">
            ...
            <div className="glass rounded-2xl p-6">
              ...
              {/* 音名 */}
              <p className="text-5xl font-bold text-center mb-4" style={{ color: devColor }}>
                {currentNote}
              </p>

              {/* 指针轨道 */}
              <div className="relative h-6 rounded-full mb-3"
              ...
              <div className="flex justify-between text-xs" style={{ color: "rgba(255,255,255,0.3)" }}>
                ...
                <span style={{ color: devColor }}>
                  {currentDeviation === Infinity ? "等待演唱…" :
                    currentDeviation <= 50 ? "✓ 在调" :
                    `偏差 ${Math.round(currentDeviation)} 音分`}
                </span>
                <span>偏高</span>
              </div>
            </div>
        ...
        {state === "processing" && (
          <div className="glass rounded-2xl p-12 flex flex-col items-center gap-5">
            <Loader2 size={40} className="text-purple-400 animate-spin" />
            <p className="font-semibold text-lg">正在生成分析报告…</p>
          </div>
        )}
